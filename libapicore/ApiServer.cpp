    ...
    const WorkingProgress& p, const SolutionStats& s, size_t index)
{
    ...
    /* Pause infos */
    if (index < p.miningIsPaused.size())
    { /* TODO: put it into pausestructure (miner->get_mining_paused_reason_string()) */
        MinigPauseReason pause_reason = miner->get_mining_paused();

        if (pause_reason == MinigPauseReason::MINING_NOT_PAUSED)
        {
            jRes["ispaused"] = false;
            jRes["pause_reason"] = "";
        }
        else
        {
            ...
            {
                jRes["pause_reason"] = seperator + "temperature";
                seperator = ",";
            }
            if (pause_reason & MinigPauseReason::MINING_PAUSED_API)
            {
                jRes["pause_reason"] = seperator + "api";
                seperator = ",";
            }
        }
    }
...
Json::Value ApiConnection::getMinerStatDetail()
{
    ...
    if (connection)
    {
        ...
        jconnection["wallet"] = connection->User();  // TODO - only if we've write access ? */
        jconnection["password"] = connection->Pass();
    }
    ...
    {
        jRes["gpu"].append(getMinerStatDetailPerMiner(p, s, i));
    }
